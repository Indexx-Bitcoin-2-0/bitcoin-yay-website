...
export default function ChatWidget() {
  ...
  function extractLinks(line: string): ReactNode[] {
  ...
  async function sendMessage(prompt?: string) {
    ...
    try {
      ...
      try {
        ...
        // ignore JSON parse errors so we can fall back to generic message
      }
      ...
      setNextSuggestionIndex((prevIndex) => {
        if (prompt && ALL_SUGGESTIONS.includes(prompt as any)) {
          const idx = ALL_SUGGESTIONS.indexOf(prompt as any);
          return (idx + 1) % ALL_SUGGESTIONS.length;
        }
        return (prevIndex + 1) % ALL_SUGGESTIONS.length;
      });
    } catch (error) {
      console.error(error);
      setMessages((prev) => [
        ...prev,
        {
          ...
          content:
            error instanceof Error && error.message
              ? `Sorry, I'm having trouble right now: ${error.message}. Please verify your Agent configuration and try again.`
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex w-[22.5rem] max-w-[92vw] flex-col overflow-hidden rounded-3xl border border-[#ff8728]/45 bg-[#181818]/95 shadow-[0_35px_65px_rgba(0,0,0,0.58)] backdrop-blur">
      ...
      <div className="flex h-[21rem] flex-col gap-3 overflow-y-auto px-4 py-4 text-[0.92rem] text-[#f6f6f6] scrollbar-hide">
        ...
        {messages.length > 0 && lastAgentMessage?.role === "agent" && (
