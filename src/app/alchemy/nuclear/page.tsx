...
import ArtImage1 from "@/assets/images/alchemy/nuclear/art-1.webp";
...
const CustomCard = ({
  ...
  return (
    <Link
      href={`/alchemy/nuclear/${planIndex}`}
      className="w-100 bg-[#9031AA] z-10 hover:scale-105 transition-all duration-300"
    >
      ...
      <div className="mt-10 bg-[#791395] flex flex-col items-center justify-center py-10">
...
export default function NuclearMiningPage() {
  const [nuclearPlans, setNuclearPlans] = useState<AlchemyConfigItem[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const fetchNuclearPlans = async () => {
      try {
        setIsLoading(true);
        setError(null);
        ...
        if (response.session?.nuclear) {
          setNuclearPlans(response.session.nuclear);
        }
      } catch (err) {
        console.error("Failed to fetch nuclear plans:", err);
        setError(
          err instanceof Error ? err.message : "Failed to fetch nuclear plans"
        );
      } finally {
        setIsLoading(false);
      }
      ...
      <div className="mx-auto mt-40 lg:mt-60 px-4 md:px-20 xl:px-40">
        <div className="flex justify-center items-center min-h-96">
          <div className="text-center">
            <p className="text-2xl font-semibold">Loading Nuclear Plans...</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="mx-auto mt-40 lg:mt-60 px-4 md:px-20 xl:px-40">
      ...
      <div className="mt-60 mb-20 mx-4 md:mx-10 lg:mx-24">
        <div className="bg-[#9031AA] rounded-2xl px-6 py-8 md:px-12 md:py-10 lg:px-16 lg:py-12">
